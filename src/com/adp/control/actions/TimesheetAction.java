...
public class TimesheetAction extends ActionSupport implements ServletContextAware{
	...
	@Autowired
	TimesheetErrorService timesheetErrorLineService ;
		...
		try 
	       {
			...
			/*
			List<TimesheetInstanceEntity> list = timesheetInstanceService.getAllInstances();
			
			boolean bool ;
		
			if(!list.isEmpty()){
			 bool = verifyNewInstance(list) ;
			}
			...
			TimesheetInstanceEntity[]  old = new TimesheetInstanceEntity[sheet.getLastRowNum()+1];
			String[] oldThirdParty = new String[sheet.getLastRowNum()+1];
			oldThirdParty[0] = r.getCell(0).getStringCellValue();
			boolean flag = false ;
			*/
	       ...
	       if(! verif(fullName,inputLines)){
        	...
        	TimesheetInputEntity timesheetInput= new TimesheetInputEntity();
        		...
        		if(!valid(month,thirdParty.getTimesheetInputs())){
		        	...
		        	//timesheetInputService.addTimesheet(timesheetInput);
        	}
        	...
        	instance.setTimesheet(timesheetInput);
            line.setTimesheetInstance(instance);
        	//timesheetInstanceService.addInstance(instance);
        	//timesheetInputLineService.addTimesheetInputLine(line);
        	...
        	if (!exist(m, thirdParty.getAssociation())) {
        		...
        		error.setErrorDescription("Mission"+m.getMissionName()+" not conform with affectation configuration ");
    			error.setErrorType("affectation TP - Mission");
    			error.setPossibleSolution("configure contributors affectations ");
    			//timesheetErrorLineService.addTimesheetErrorLine(error);
	...
	public Set<TimesheetErrorEntity> Errors (ThirdPartyEntity thirdParty , Date workDate, float hours , MissionEntity m, TimesheetInstanceEntity inst) throws ADPException{
	...
	//2. comparing period on mission to working date 	
        ...
        if(workDate.compareTo(startDate) * workDate.compareTo(endDate) > 0) {
			 error.setErrorDescription("Mission "+m.getMissionName()+"  not allowed for thirdParty"+ thirdParty.getFullName() +" on date "+workDate);
			 error.setErrorType("affectation TP - Mission");
	...
	//3. nb hours < 8 for a working date
		 if(hours<8) {
 			...
 			error = new TimesheetErrorEntity("Hours number for mission "+m.getMissionName()+" is less than 8h for working Date"+workDate, "Hours Per Mission", "Rework issue ");
 			error.setTimesheetInstance(inst);
 			//timesheetErrorLineService.addTimesheetErrorLine(error);
 			errors.add(error);
...
public String viewReport() throws ParseException, ADPException {
		...
		try {
			if(selectedMonth.equals("0")&& selectedProject.equals("0") 
					&& selectedTeam.equals("0") && thirdPartyName.equals("0")){
				...
				Date latest = null ;
				List<TimesheetInstanceEntity> instances = timesheetInstanceService.getInstancesByMonth(Integer.parseInt(selectedMonth));		
				for(TimesheetInstanceEntity inst : instances){
					if(latest == null || inst.getImportDate().compareTo(latest) > 0 ){
						latest=inst.getImportDate() ;
						timesheetLatestInstance = inst ;
					}
